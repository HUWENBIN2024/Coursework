...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    out = np.dot(x.reshape(x.shape[0], -1), w) + b.reshape(-1, b.shape[0])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(F):
      for c in range(C):
        for i in range(HH):
          for j in range(WW):
            dw[f, c, i, j] = np.sum(x_padded[:, c, i: i + H_  * stride: stride, j: j + W_ * stride: stride] * dout[:, f, :, :])
    ...
    dx = conv_forward_naive(new_out, w_rot, b = np.zeros(C,), conv_param={'stride': 1, 'pad': 0})[0][:,:,pad:-pad,pad:-pad]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_):
      for j in range(W_):
        mask = x[:,:,i*s : i*s+h, j*s : j*s + w]
        max_position = (mask == np.max(mask, axis=(2,3), keepdims=True))
        dx[:,:,i*s : i*s+h, j*s : j*s + w] += max_position * dout[:,:,i,j][:, :, None, None]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_ = np.moveaxis(x, 1, 3).reshape(N*H*W, C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_ = np.moveaxis(dout, 1, 3).reshape(N*H*W, C)
