...
parser.add_option("--model_path",dest="model_path", default=None)
...
if options.model_path == None:
	parser.error("what model weight is used for testing, please input")
...
# C.model_path = "resnet50_result/model_frcnn_0039.hdf5"
...
for idx, img_data in enumerate(test_imgs):
	...
	img = cv2.imread('testdataset/'+filepath)
	st = time.time()
	...
	# print("mean elapsed time is",np.mean(times))
	allmaps.append(np.mean(np.array(all_aps)))

	
mAP = [mAP for mAP in allmaps if str(mAP)!='nan']
mean_average_prec = round(np.mean(np.array(mAP)), 3)
print('Overall mean average percision (mAP) is %0.3f'%( mean_average_prec))
# print("Overall mean average percision (mAP) is", np.mean(allmaps))
ending = time.time() - begin
print('Mean Elapsed time = ',ending/197)
print("mean FPS=", 197/ending)
