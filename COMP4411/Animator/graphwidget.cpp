	...
	if (m_bHasEvent) {
		...
		switch (m_iEventToDo) {
			case LEFT_MOUSE_DOWN:
				if (VAL(EDITCR)) {
					selectInnerPt(m_iMouseX, m_iMouseY);
				}
				if (innerOn) break;
				selectAddCtrlPt(m_iMouseX, m_iMouseY);
				break;
			case LEFT_MOUSE_DRAG:
				if (!innerOn) dragCtrlPt(m_iMouseX, m_iMouseY);
				if (VAL(EDITCR) && innerOn) dragInnerPt(m_iMouseX, m_iMouseY);
				break;
			case LEFT_MOUSE_UP:
				innerOn = false;
				break;
...
void GraphWidget::selectInnerPt(const int iMouseX, const int iMouseY) {
	if (m_iCurrCurve >= 0) {
		Point ptMouse(iMouseX, iMouseY);
		m_ptDragStart = ptMouse;
		Point ptInnerPt;
		...
		Point ptMouseInCurveCoord = windowToCurve(m_iCurrCurve, ptMouse);
		Point CtrlPt = m_pcrvvCurves[m_iCurrCurve]->getClosestInnerPoint(ptMouseInCurveCoord, ptInnerPt);

		Point ptInnerPtInWindowCoord = curveToWindow(m_iCurrCurve, ptInnerPt);

		if (fabs(ptInnerPtInWindowCoord.x - ptMouse.x) * 2 <= PICK_WINDOW_SIZE &&
			fabs(ptInnerPtInWindowCoord.y - ptMouse.y) * 2 <= PICK_WINDOW_SIZE) {
			// select the inner point
			currInnerPtCtrl = CtrlPt;
			currInnerPt = ptInnerPt;
			innerOn = true;
			return;
		}
		innerOn = false;
...
void GraphWidget::dragInnerPt(const int iMouseX, const int iMouseY) {
	if (m_ivActiveCurves.size() > 0) {
		...
		for (int i = 0; i < m_ivActiveCurves.size(); ++i) {
			...
			m_pcrvvCurves[iCurve]->moveInnerPoints(currInnerPtCtrl, currInnerPt, ptOffset,
				m_cdvCurveDomains[iCurve].minimum(), m_cdvCurveDomains[iCurve].maximum());
			currInnerPt = ptMouseInCurveCoord;
		}
...
void GraphWidget::drawCurve(int iCurve, int iColor) const
{
	...
	if (VAL(EDITCR)) m_pcrvvCurves[iCurve]->drawInnerPoints();

	glColor3f(1.0f, 1.0f, 1.0f); // white
	for (int i = 0; i < m_ivvCurrCtrlPts[iCurve].size(); ++i) {
		m_pcrvvCurves[iCurve]->drawControlPoint(m_ivvCurrCtrlPts[iCurve][i]);
		if (VAL(EDITCR)) m_pcrvvCurves[iCurve]->drawInnerPoint(m_ivvCurrCtrlPts[iCurve][i]);
	}
	m_pcrvvCurves[iCurve]->resetPrevTau();
