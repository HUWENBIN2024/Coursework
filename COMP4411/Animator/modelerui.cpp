...
inline void ModelerUI::cb_clearSim_i(Fl_Button* o, void* v)
{
	...
	if (ps) {
		if (ps->getType() == SampleParticleSystems::SPRING) {
			dynamic_cast<SpringParticleSystem*>(ps)->clearBaked();
		}
		else if (ps->getType() == SampleParticleSystems::CLOTH) {
			dynamic_cast<ClothParticleSystem*>(ps)->clearBaked();
		}
		else {
			ps->clearBaked();
		}
...
{
	m_iFps = iFps;
	ModelerApplication::Instance()->GetParticleSystem()->setBakeFps(iFps);
}
...
m_bSaveMovie(false)
{
	...
	// set "noCurve" boolean values
	// if you want some control to be not affected by its curve, then write "noCurve[ <CONTROL_ID> ] = true;" below.
	noCurve[CR_TENSION] = true;
	noCurve[EULER_PRECISION] = true;
	noCurve[RK] = true;
	noCurve[PS] = true;
	noCurve[WHEEL_QUALITY] = true;
	noCurve[SADDLE_QUALITY] = true;
	noCurve[WIND] = true;
	noCurve[EDITCR] = true;
	noCurve[FLOCKSPAWN] = true;
}
