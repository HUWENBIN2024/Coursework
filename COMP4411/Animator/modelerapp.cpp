...
{
	ps = s;
	if (pss[s->getType()]) delete pss[s->getType()];
	pss[s->getType()] = s;
}

void ModelerApplication::SetParticleSystem(SampleParticleSystems type) {
	if (pss[type]) ps = pss[type];
	else cout << "Particle System of this type do not exist" << endl;
}
